...
- Container images run as `root`. Mount the private key at `/root/.ssh/id_rsa` or use `--libvirt.ssh-key-file=/path/to/key`.
...
- For custom key names, use `--libvirt.ssh-key-file=/path/to/key` (e.g. `--libvirt.ssh-key-file=/etc/libvirt-exporter/id_rsa_libvirt`).
