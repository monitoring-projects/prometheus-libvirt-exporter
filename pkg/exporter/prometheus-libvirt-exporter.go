...
func NewLibvirtExporter(uri string, driver libvirt.ConnectURI, logger *slog.Logger, timeout time.Duration, maxConcurrentCollects int, sshKeyFile string) (*LibvirtExporter, error) {
...
func dialerForURI(uri string, sshKeyFile string) (socket.Dialer, error) {
	if strings.HasPrefix(uri, "/") {
	...
	case "qemu+ssh":
		...
		if sshKeyFile != "" {
			opts = append(opts, dialers.UseKeyFile(sshKeyFile))
		}
		if port := u.Port(); port != "" {
...
func (e *LibvirtExporter) Collect(ch chan<- prometheus.Metric) {
	if err := CollectFromLibvirt(ch, e.uri, e.driver, e.logger, e.timeout, e.maxConcurrentCollects, e.sshKeyFile); err != nil {
...
func CollectFromLibvirt(ch chan<- prometheus.Metric, uri string, driver libvirt.ConnectURI, logger *slog.Logger, timeout time.Duration, maxConcurrentCollects int, sshKeyFile string) (err error) {
	var (
		wg    sync.WaitGroup
		pools []libvirt.StoragePool
	)

	dialer, err := dialerForURI(uri, sshKeyFile)
	if err != nil {
...
func (e *LibvirtExporter) HealthHandler(w http.ResponseWriter, r *http.Request) {
	dialer, err := dialerForURI(e.uri, e.sshKeyFile)
	if err != nil {
