...
// discoverHandler returns the targets file in Prometheus file_sd / HTTP SD
// JSON format, optionally filtered by a ?targets_file= query parameter.
func discoverHandler(logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			logger.Error("Failed to read targets file for /discover", "error", err, "file", targetsFile)
		...
		if err := yaml.Unmarshal(data, &groups); err != nil {
			logger.Error("Failed to parse targets file for /discover", "error", err, "file", targetsFile)
		...
		for _, g := range groups {
			entry := map[string]any{"targets": g.Targets}
			if len(g.Labels) > 0 {
		...
		if err := enc.Encode(result); err != nil {
			logger.Error("Failed to encode /discover response", "error", err)
		}
	}
}
...
func clusterHandler(logger *slog.Logger, timeout time.Duration, sshKeyFile string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		start := time.Now()
		...
		for i, j := range jobs {
			...
			go func(i int, j job) {
				defer wg.Done()
				defer func() { <-sem }()
				nodes[i] = scrapeNode(j.connectionURI, j.labels, logger, timeout, sshKeyFile)
			}(i, j)
		}
		wg.Wait()
...
func scrapeNode(connectionURI string, labels map[string]string, logger *slog.Logger, timeout time.Duration, sshKeyFile string) *NodeState {
	node := &NodeState{
		Host:          connectionURI,
	...
	libvirtExporter, err := exporter.NewLibvirtExporter(connectionURI, libvirt.ConnectURI(connectionURI), logger, timeout, 4, sshKeyFile)
	if err != nil {
